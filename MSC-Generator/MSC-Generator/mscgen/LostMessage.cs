...
namespace mscElements
{
	...
	/// </summary>
	public partial class LostMessage : MSCItem
	{
		...
		public LostMessage(uint fileLine, string name, uint line, int process)
		{
			this.mName 				= name;
			this.mGate 				= "";
			this.mLine 				= line;
		...
		public LostMessage(uint fileLine, string name, string gate, uint line, int process)
		{
			this.mName 				= name;
			this.mGate 				= gate;
			this.mLine 				= line;
		...
		public LostMessage(uint fileLine, string name, uint line, int process, MessagePos placement)
		{
			this.mName 				= name;
			this.mGate 				= "";
			this.mLine 				= line;
		...
		public LostMessage(uint fileLine, string name, string gate, uint line, int process, MessagePos placement)
		{
			this.mName 				= name;
			this.mGate 				= gate;
			this.mLine 				= line;
		...
		public void DrawItem(Graphics drawDestination, float xPos, float yPos, float lineHeight)
		{
			...
			if (mMscStyle == MscStyle.SDL){
				if (this.mPos==MessagePos.Right){
					// Message
					capPolygon[0] = new PointF(xPos+MSCItem.ItemLayoutSize.Width/2-rGate, yPos+lineHeight);
					capPolygon[1] = new PointF(xPos+MSCItem.ItemLayoutSize.Width/2-rGate-8, yPos+lineHeight-4);
					capPolygon[2] = new PointF(xPos+MSCItem.ItemLayoutSize.Width/2-rGate-8, yPos+lineHeight+4);
				...
				if (this.mPos==MessagePos.Right){
					...
					drawDestination.DrawString(mName,mItemFont,mItemStringBrush,itemBox,itemStringFormat);
					drawDestination.DrawLine(mItemPen,xPos, yPos + lineHeight, xPos+MSCItem.ItemLayoutSize.Width/2-rGate,yPos + lineHeight);
					drawDestination.DrawLine(mItemPen,xPos+MSCItem.ItemLayoutSize.Width/2-rGate, yPos + lineHeight, xPos+MSCItem.ItemLayoutSize.Width/2-rGate-8,yPos+lineHeight-4);
					drawDestination.DrawLine(mItemPen,xPos+MSCItem.ItemLayoutSize.Width/2-rGate, yPos + lineHeight, xPos+MSCItem.ItemLayoutSize.Width/2-rGate-8,yPos+lineHeight+4);
					// Gate
					itemStringFormat.Alignment = StringAlignment.Far;
					itemBox = new RectangleF(xPos, yPos+lineHeight+rGate, MSCItem.ItemLayoutSize.Width/2+rGate, itemGateSize.Height);
