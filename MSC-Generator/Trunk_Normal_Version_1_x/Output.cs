...
namespace nGenerator
{
	...
	public partial class Output : GUI
	{
		...
		private bool 			mRedrawOutput 			= true; 	// if false no output redraw
		...
		public Output(string[] args)
		{
			...
			fileFilter = strings.GetString("FileFilter");
  			try{
				...
				if (args.Length ==0){
					Start start = new Start(license.company1, license.company2, license.licenseType, license.licenseNumber, license.valid);
					start.ShowDialog();	
				}
				if (args.Length == 1){
				...
				if (args.Length == 2){
					if ((license.valid == ValidResult.OutOfDate)||(license.valid == ValidResult.NoLicensed)){
						this.LoadFile(args[0],true);
					}
					else{
						string directory;
		...
		private void CheckLicense(){
			if (license.valid == ValidResult.OutOfDate){
				MessageBox.Show(string.Format(strings.GetString("imageBitmapLicenseExpired"), license.licenseDate), 
				                strings.GetString("LicenseInfo"),
				                MessageBoxButtons.OK, MessageBoxIcon.Information);
				this.umnuPrint.Enabled = false;
				this.umnuExport.Enabled = false;
				this.umnuExport.ToolTipText = strings.GetString("ExportingOutOfDate");
				this.umnuPrint.ToolTipText = strings.GetString("ExportingOutOfDate");
				this.SetIsRegistered(false);
			}
			else if(license.valid == ValidResult.NoLicensed){
				this.umnuExport.Text = strings.GetString("ExportingOnlyInFullVersion");
				this.umnuExport.Enabled = false;
				this.umnuPrint.Text = strings.GetString("PrintingOnlyInFullVersion");
				this.umnuPrint.Enabled = false;
				this.SetIsRegistered (false);
				
				//this
				//this.umnuExport.Visible = false;
				//this.tlbbSave.Visible = false;				
			}
 ...
 * ***********************************************/
			...
			if (startPos>-1){
				...
				this.rtbMscEditor.Select(startPos,endPos-startPos);
				if (true == Worksheet.DrawFootLine) this.rtbMscEditor.SelectedText = temp.ToString();
				else this.rtbMscEditor.SelectedText="";
			}
 ...
 * ***********************************************/
		...
		{
			if (this.RedrawOutput == false) return;
			...
			if (pages == 0) return;																// if nothing to do, do nothing
			MSCItem item = null;
...
//			pages = this.GetPages();
			...
			if (this.lstPreview.Items.Count > 0)							// select first picture of image list
				this.lstPreview.SelectedIndex = 0;
			if (mscPreview!=null) mscPreview.Dispose();
			if (previewDestination!=null) previewDestination.Dispose();
		}
		...
		{
			if (oldRepertoryStyle == MSCItem.Style) return;								// dont redraw if style not changed
...
//			System.GC.Collect();
		...
		protected override void PrintDocumentPrintPage(object sender, PrintPageEventArgs e)
		{
			...
			if (maxPage==0) maxPage=pages;												// if not max page selected, print to last page
		...
		public override void LogClick(){			// log item was clicked so mark the line in editor
		...
		public override void LogDoubleClick(){		// log item was clicked so mark the line in editor and hide log window
			if (this.tabWork.SelectedIndex == 0)
				this.tabWork.SelectedIndex = 1;
			this.BringToFront();
			try{
		...
		public override void LogSelectedIndexChanged(){	 // selection in log window was changed so mark the new line in editor
			if (this.tabWork.SelectedIndex == 0){
				this.tabWork.SelectedIndex = 1;
				log.BringToFront();
			}
