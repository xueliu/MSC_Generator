...
namespace nGenerator
{
	...
	partial class Generator
	{
		...
		public void DrawPage(Graphics drawDestination, Worksheet worksheet, uint page)
		{
				...
				for(uint i=0;i<items.Count;i++){										// store the maximum line height of each msc line
					...
					if (enumerator.Current is Comment)
					{
						if (((Comment)enumerator.Current).Position == CommentPos.OverAll){
							...
							if ((((Comment)enumerator.Current).Process == Generator.ENV_LEFT)||(((Comment)enumerator.Current).Process == Generator.ENV_RIGHT))
								...
								mDiagramLines[((Comment)enumerator.Current).Line].Height = Math.Max(mDiagramLines[((Comment)enumerator.Current).Line].Height,((Comment)enumerator.Current).GetHeight(drawDestination,GetInstanceXPos(((Comment) enumerator.Current).Process), worksheet.LeftMargin,worksheet.Width-worksheet.RightMargin, MSC.LeftMargin + ((Process)processes[((Comment)enumerator.Current).Process]).LeftRand, MSC.RightMargin + ((Process)processes[((Comment)enumerator.Current).Process]).RightRand));
						}
			...
			for(uint i=0;i<items.Count;i++){
					...
					if (((Comment) enumerator.Current).Line == line){
						if (((Comment)enumerator.Current).Position == CommentPos.OverAll){
							...
							if ((((Comment)enumerator.Current).Process == Generator.ENV_LEFT)||(((Comment)enumerator.Current).Process == Generator.ENV_RIGHT))
								...
								((Comment) enumerator.Current).DrawItem(drawDestination,GetInstanceXPos(((Comment) enumerator.Current).Process), worksheet.LeftMargin,worksheet.Width-worksheet.RightMargin,yPos, MSC.LeftMargin + ((Process)processes[((Comment)enumerator.Current).Process]).LeftRand, MSC.RightMargin + ((Process)processes[((Comment)enumerator.Current).Process]).RightRand);
						}
					}
				}
				if (enumerator.Current is LineComment)
				...
				if (enumerator.Current is ProcessCreate)
				{
					if (((ProcessCreate) enumerator.Current).Line == line)
						((ProcessCreate) enumerator.Current).DrawItem(drawDestination,GetInstanceXPos(((ProcessCreate) enumerator.Current).Source),GetInstanceXPos(((ProcessCreate) enumerator.Current).Destination),yPos,mDiagramLines[line].Height, GetLifeLineStyle(((ProcessCreate) enumerator.Current).Source, line), GetLifeLineStyle(((ProcessCreate) enumerator.Current).Destination, line));
				}
				if (enumerator.Current is ProcessRegion)
				{
