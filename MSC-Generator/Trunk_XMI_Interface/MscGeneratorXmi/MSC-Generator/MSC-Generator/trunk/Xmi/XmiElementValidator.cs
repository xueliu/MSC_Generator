...
namespace xmi
{
	...
	public static class XmiElementValidator
	{
		...
		public static bool IsCorrectXmiVersion(XmlElement element,string assertXmiVersion)
		{
			bool isCorrectXmiVersion=false;
			string actualXmiVersion=element.GetAttribute(UmlModel.XMI_VERSION_ATTR_COMPLETE_NAME);
			
			if(assertXmiVersion.Equals(actualXmiVersion))
			{
				isCorrectXmiVersion=true;
			}
