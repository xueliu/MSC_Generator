...
namespace MscItemProp
{
	...
	public class TimeoutEndProp : Property
	{
		public TimeoutEndProp()
		{
			//
		...
		private void InitializeComponent() {
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(TimeoutEndProp));
		...
		public override string PropText{
			get{
				...
				int dir = mEditorText.LastIndexOf(',');
				if (dir != pos){
					dir++;
					int end = mEditorText.IndexOf(';',dir);
					if (end == -1) end = mEditorText.Length;
					string t = mEditorText.Substring(dir,end-dir).Trim().ToLower();
					if ((t=="l")||(t=="r"))
						d = "," + t;
				}
				string command = mEditorText.Substring(0,pos).TrimEnd();
				command += ", \"" + this.TimeoutText + "\"" + d;
